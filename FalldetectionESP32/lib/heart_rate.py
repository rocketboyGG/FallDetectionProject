...
class HeartRateSensor:
    def __init__(self, i2c):
        self.heart_sensor = MAX30102(i2c=i2c)
        # Scan I2C bus to ensure that the sensor is connected
        if self.heart_sensor.i2c_address not in i2c.scan():
            print("Sensor not found.")
        elif not (self.heart_sensor.check_part_id()):
        ...
        self.heart_sensor.setup_sensor()

        # Set the sample rate to 400: 400 samples/s are collected by the sensor
        sensor_sample_rate = 400
        self.heart_sensor.set_sample_rate(sensor_sample_rate)

        # Set the number of samples to be averaged per each reading
        sensor_fifo_average = 8
        self.heart_sensor.set_fifo_average(sensor_fifo_average)

        # Set LED brightness to a medium value
        self.heart_sensor.set_active_leds_amplitude(MAX30105_PULSE_AMP_MEDIUM)
    ...
    def run_sensor(self):
        try:
            self.heart_sensor.check()

            # Check if the storage contains available samples
            if self.heart_sensor.available():
                # Access the storage FIFO and gather the readings (integers)
                red_reading = self.heart_sensor.pop_red_from_storage()
                ir_reading = self.heart_sensor.pop_ir_from_storage()
